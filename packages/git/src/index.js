...
function classifyChange(indexStatus, workTreeStatus) {
...
function readNewFileContent(fullPath) {
    ...
    const lines = visible.map((content, index) => ({
        type: "add",
        content,
        oldLineNo: null,
        newLineNo: index + 1
...
export class GitCliService {
    ...
    async getDiff(projectPath, filePath, mode, staged = false) {
        if (!filePath) {
    ...
    async getHistory(projectPath, filePath) {
        if (!filePath) {
            return [];
        }
        const output = await runGit(projectPath, ["log", "-n", "5", "--format=%H%x1f%an%x1f%ad%x1f%s", "--follow", "--", filePath], "");
        if (!output) {
            return [];
        }
        return output.split("\n").map((line) => {
    ...
    async getCommitDiff(projectPath, commitHash, filePath, mode) {
        if (!filePath || !commitHash) {
            return { mode, filePath, lines: [] };
        }
        const output = await runGit(projectPath, ["show", commitHash, "--", filePath], "");
        return { mode, filePath, lines: parseUnifiedDiff(output) };
    }
    async getStagedDiff(projectPath) {
    ...
    addToGitIgnore(projectPath, filePath) {
